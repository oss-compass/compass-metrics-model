"""
评估模型：核心开发者留存率 (Core Retention Rate)

度量指标（周期：月、季度、年）：
- 组织/个人 × 代码/Issue 核心开发者（含管理者）留存率（4 项）
"""
...
class CoreRetentionMetricsModel(BaseMetricsModel):
    """
    核心开发者留存率指标模型

    对应 developer_metrics_v2：org_code_core_retention_by_period 等 4 个函数
    """
    ...
    def __init__(self, repo_index, git_index, issue_index, pr_index, issue_comments_index, pr_comments_index,
        ...
        model_name = 'Core Retention'
        metrics_weights_thresholds = {
            "org_code_core_retention_by_period": {"weight": W, "threshold": None},
            "org_issue_core_retention_by_period": {"weight": W, "threshold": None},
            "individual_code_core_retention_by_period": {"weight": W, "threshold": None},
            "individual_issue_core_retention_by_period": {"weight": W, "threshold": None},
        }
