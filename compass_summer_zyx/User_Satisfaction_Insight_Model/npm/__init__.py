...
version: V1.0
Author: zyx
Date: 2024-09-10 02:12:51
LastEditors: zyx
LastEditTime: 2024-09-29 11:17:52
'''
import sys
sys.path.append(".")
from npm.bug_issue import bug_issue
from npm.comprehensive_downloads import comprehensive_downloads
from npm.software_universality import get_npm_dependentCount
from npm.star import npm_star
from npm.stability_of_version_updates import stability_of_version_updates
...
WEIGHT_MATRIX = read_json_file(r"npm/weight.json")
GITHUB_BENCHMARK_MATRIX =  read_json_file(r"npm/github_normalize_json/benchmark.json")
GITHUB_MAX_SCORE = read_json_file(r"npm/github_normalize_json/max_score.json")
GITHUB_MIN_SCORE =  read_json_file(r"npm/github_normalize_json/min_score.json")
GITEE_BENCHMARK_MATRIX =  read_json_file(r"npm/gitee_normalize_json/benchmark.json")
GITEE_MAX_SCORE = read_json_file(r"npm/gitee_normalize_json/max_score.json")
GITEE_MIN_SCORE =  read_json_file(r"npm/gitee_normalize_json/min_score.json")
...
class NPM_USER_MODEL:
    
    def __init__(self,url) -> None:

        self.url = url
        self.flag,self.package_name,self.user = self.get_package_user()
    ...
    def get_user_score(self):
        ...
        save_name_score = "npm_score_"+self.flag+"_"+self.package_name+"_"+self.user+".json"
        save_name_metric = "npm_metric_"+self.flag+"_"+self.package_name+"_"+self.user+".json"

        # 数据初始化
        metrics = {
            "bug_issue" : bug_issue(self.url, self.beginDate, self.endDate),
            "download" : comprehensive_downloads(self.package_name,self.beginDate,self.endDate),
            "software_universality" : get_npm_dependentCount(self.package_name),
            "star" : npm_star(self.flag,self.user,self.package_name),
...
if __name__=="__main__":
    a = NPM_USER_MODEL("https://github.com/jquery/jquery")
    a.get_user_score()